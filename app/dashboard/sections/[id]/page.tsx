...
export default function SectionProductsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => { setPage(1); }, [search, catFilter, tagFilter, showInSection]);
  ...
  if (!section) return <div style={{ padding: 40 }}>Section not found. <Link href="/dashboard/sections">← Back</Link></div>;

  return (
    <div style={{ padding: '32px', maxWidth: 1400, margin: '0 auto' }}>
      <Link href="/dashboard/sections" style={{ fontSize: 13, color: 'var(--text-muted)', display: 'block', marginBottom: 12 }}>← Sections</Link>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>{section.name}</h1>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
            {inSection.size} product{inSection.size !== 1 ? 's' : ''} in this section · {filtered.length} shown
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)} style={{
          padding: '9px 12px', fontSize: 13, border: '0.5px solid var(--border)',
          borderRadius: 8, background: 'white', outline: 'none', cursor: 'pointer'
        }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {selected.size > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', background: '#2C2420', borderRadius: 8 }}>
            ...
            <button onClick={() => batchAction('add')} disabled={saving} style={{ background: '#3B6D11', border: 'none', borderRadius: 6, padding: '5px 12px', fontSize: 12, cursor: 'pointer', color: 'white', fontWeight: 500 }}>
              + Add to {section.name}
            </button>
            <button onClick={() => batchAction('remove')} disabled={saving} style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: 6, padding: '5px 12px', fontSize: 12, cursor: 'pointer', color: 'white' }}>
              − Remove
            </button>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, fontSize: 11, color: 'var(--text-muted)' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 12, height: 12, borderRadius: 2, background: '#3B6D11', display: 'inline-block' }} /> In this section
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 12, height: 12, borderRadius: 2, background: 'var(--accent)', display: 'inline-block' }} /> Selected for batch action
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          Click image to toggle in/out of section instantly
        </span>
      </div>
          ...
          return (
            <div key={p.id} style={{
              ...
              <div
                ...
                style={{ position: 'absolute', top: 6, left: 6, zIndex: 3, cursor: 'pointer' }}
              >
                <div style={{
                  width: 18, height: 18, borderRadius: 4,
                  background: isSelected ? 'var(--accent)' : 'rgba(255,255,255,0.9)',
                  border: `1.5px solid ${isSelected ? 'var(--accent)' : 'rgba(0,0,0,0.2)'}`,
              ...
              {/* In-section indicator */}
              {isInSection && (
                <div style={{
                  position: 'absolute', top: 6, right: 6, zIndex: 3,
                  background: '#3B6D11', borderRadius: 4,
                  width: 18, height: 18,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <div
                ...
                style={{
                  aspectRatio: '3/4', background: p.bg_color,
              ...
              <div style={{ padding: '8px 10px 10px' }}>
                <div style={{ fontSize: 11, fontWeight: 500, lineHeight: 1.3, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {cleanTitle(p.title)}
                </div>
                <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'capitalize' }}>
                  {p.category} · ${p.price_digital.toFixed(2)}
                </div>
              </div>
            </div>
          );
